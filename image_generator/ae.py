    ...
    class PatchEncoder(torch.nn.Module):
        ...
        def __init__(self, patch_size: int, latent_dim: int, bottleneck: int):
            ...
            self.encoder = torch.nn.Sequential(
                torch.nn.Conv2d(latent_dim, bottleneck, 3, padding=1, stride=1),
                torch.nn.GELU(),
                torch.nn.Conv2d(bottleneck, bottleneck, 3, padding=1 , stride=1)
            )

        def forward(self, x: torch.Tensor) -> torch.Tensor:
              x = self.patchify(x)  # (B, H//p, W//p, latent_dim)
              x = hwc_to_chw(x)  # Convert to channels-first for Conv2d
              x = self.encoder(x)
              return chw_to_hwc(x)
    ...
    class PatchDecoder(torch.nn.Module):
        def __init__(self, patch_size: int, latent_dim: int, bottleneck: int):
            ...
            self.decoder = torch.nn.Sequential(
                torch.nn.Conv2d(bottleneck, latent_dim, 3, padding=1, stride=1),
                torch.nn.GELU(),
                torch.nn.Conv2d(latent_dim, latent_dim, 3, padding=1 , stride=1)
            )
        ...
        def forward(self, x: torch.Tensor) -> torch.Tensor:
            x = hwc_to_chw(x)  # Convert to channels-first for Conv2d
            x = self.decoder(x)
            x = chw_to_hwc(x)  # Convert back to channels-last
